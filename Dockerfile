...
RUN apk update && apk --no-cache upgrade && apk --no-cache add ca-certificates gettext flite sox shadow python3 py3-psycopg2 dumb-init bash git curl uwsgi-python3 uwsgi-http uwsgi-spooler uwsgi-cache uwsgi-router_cache uwsgi-router_static && pip3 install --upgrade pip
